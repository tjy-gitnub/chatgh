...
updateStreamingMessage(content) {
    ...
    const processedContent = this.processThinkContent(content);
    const $content = $messageDiv.find('.message-content');
    
    // 更新内容
    $content.html(marked.parse(processedContent));
...
showError(message) {
    ...
    saveHistory() {
        try {
            ...
            localStorage.setItem('chatHistory', JSON.stringify(historyWithFormat));
        } catch (error) {
    ...
    renderChatHistory() {
        ...
        this.chatHistory.forEach(message => {
            ...
            // 检查是否需要显示时间戳
            if (!lastTimestamp || (currentTime - lastTimestamp) > 180000) { // 3分钟 = 180000毫秒
                const $timeDiv = $('<div>')
